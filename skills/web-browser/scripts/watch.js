...
import { connect, getAutomationLogDir, getAutomationProfileDir, normalizeProfileName } from './lib/cdp.mjs';
import { isProcessAlive } from './lib/automation-session.mjs';

const PROFILE = normalizeProfileName(process.env.CDP_PROFILE);
const PROFILE_DIR = getAutomationProfileDir(PROFILE);
const LOG_ROOT = getAutomationLogDir(PROFILE);
const PID_FILE = join(PROFILE_DIR, 'watch.pid');
...
if (existsSync(PID_FILE)) {
  try {
    ...
    if (existing && isProcessAlive(existing)) {
      console.log(`✓ watch already running for profile ${PROFILE}`);
      process.exit(0);
    }
  } catch {
...
async function enableSession(cdp, sessionId) {
  await cdp.enableDomains(sessionId, ['Runtime', 'Log', 'Network', 'Page']);
}
...
async function main() {
  const cdp = await connect({ timeout: 5000, profile: PROFILE });
  ...
  console.log(`✓ watch started for profile ${PROFILE}`);
}

...
// Keypress: parse "Ctrl+Enter", "Shift+A", "x", "ArrowRight", etc.
function resolveKeypressParams(combo) {
  ...
  if (key === '') {
    ...
    for (let i = 0; i < parts.length - 1; i++) {
      if (parts[i]) modifiers.push(parts[i].trim());
    }
  } else {
    for (let i = 0; i < parts.length - 1; i++) {
      if (parts[i]) modifiers.push(parts[i].trim());
  ...
  if (key.length === 1) {
    ...
    else if (ch === '+') code = 'Equal';
    else if (ch === '=') code = 'Equal';
    else if (ch === '-') code = 'Minus';
    else if (ch === '_') code = 'Minus';
...
async function keypressStr(cdp, sid, combo) {
  const { key, code, modifiers } = resolveKeypressParams(combo);
  const downParams = { type: 'keyDown', key, modifiers };
  const upParams = { type: 'keyUp', key, modifiers };
  if (code) {
    downParams.code = code;
    upParams.code = code;
  }
...
Usage: cdp <command> [args]
  ...
  keypress <target> <combo>         Press a key or key combo, e.g. x, Enter, Ctrl+Enter, Shift+Tab
                                     Fires keyDown/keyUp through the renderer (triggers keydown listeners)
...
async function main() {
  ...
  if (cmd === 'eval') {
    ...
    if (!cmdArgs[0]) { console.error('Error: key combo required (e.g. x, Ctrl+Enter, Shift+Tab)'); process.exit(1); }
  } else if (cmd === 'evalraw') {
